/**
 * Autocomplete catalog: representative key actives & bases (not label-perfect INCI).
 * Many entries are curated from SKUs commonly sold at Ulta and/or Sephora in the US;
 * stock and formulas change — always read your own packaging. Catalog is expanded over time.
 */

export type CatalogProduct = {
  id: string;
  name: string;
  brand: string;
  aliases: string[];
  keyActives: string[];
  mainIngredients: string[];
  /** Typical US retail presence for discovery search (not a live inventory guarantee). */
  retailers?: ("ulta" | "sephora")[];
};

export const PRODUCT_CATALOG: CatalogProduct[] = [
  {
    id: "cerave-moisturizing-cream",
    name: "Moisturizing Cream",
    brand: "CeraVe",
    aliases: ["cerave cream", "tub", "ceramide cream"],
    keyActives: ["Ceramides NP, AP, EOP", "Hyaluronic acid"],
    mainIngredients: [
      "Petrolatum",
      "Dimethicone",
      "Glycerin",
      "Cetearyl alcohol",
      "Caprylic/capric triglyceride",
    ],
  },
  {
    id: "cerave-pm",
    name: "PM Facial Moisturizing Lotion",
    brand: "CeraVe",
    aliases: ["cerave pm", "pm lotion"],
    keyActives: ["Niacinamide", "Ceramides", "Hyaluronic acid"],
    mainIngredients: [
      "Caprylic/capric triglyceride",
      "Glycerin",
      "Niacinamide",
      "Cetearyl alcohol",
    ],
  },
  {
    id: "to-niacinamide",
    name: "Niacinamide 10% + Zinc 1%",
    brand: "The Ordinary",
    aliases: ["ordinary niacinamide", "to niacinamide", "niacinamide zinc"],
    keyActives: ["Niacinamide (10%)", "Zinc PCA (1%)"],
    mainIngredients: ["Pentylene glycol", "Dimethyl isosorbide", "Tamarind seed gum"],
  },
  {
    id: "to-retinol-squalane",
    name: "Retinol 0.5% in Squalane",
    brand: "The Ordinary",
    aliases: ["ordinary retinol", "to retinol", "retinol squalane"],
    keyActives: ["Retinol (0.5%)"],
    mainIngredients: ["Squalane", "Caprylic/capric triglyceride", "Bisabolol"],
  },
  {
    id: "to-vitamin-c-suspension",
    name: "Vitamin C Suspension 23% + HA Spheres 2%",
    brand: "The Ordinary",
    aliases: ["ordinary vitamin c", "ascorbic acid suspension", "to vit c"],
    keyActives: ["L-Ascorbic acid (23%)", "Sodium hyaluronate"],
    mainIngredients: ["Squalane", "Coconut alkanes", "Glycerin"],
  },
  {
    id: "to-glycolic-toner",
    name: "Glycolic Acid 7% Toning Solution",
    brand: "The Ordinary",
    aliases: ["ordinary glycolic", "glycolic toner", "7% toner"],
    keyActives: ["Glycolic acid (7%)"],
    mainIngredients: ["Aloe leaf water", "Ginseng root", "Tasmanian pepperberry"],
  },
  {
    id: "paula-bha",
    name: "Skin Perfecting 2% BHA Liquid Exfoliant",
    brand: "Paula's Choice",
    aliases: ["bha liquid", "paulas bha", "2% bha", "salicylic paula"],
    keyActives: ["Salicylic acid (2%)"],
    mainIngredients: ["Green tea extract", "Methylpropanediol", "Butylene glycol"],
  },
  {
    id: "paula-omega",
    name: "Omega+ Complex Moisturizer",
    brand: "Paula's Choice",
    aliases: ["omega complex", "paulas omega"],
    keyActives: ["Ceramides", "Cholesterol", "Omega fatty acids"],
    mainIngredients: ["Shea butter", "Glycerin", "Caprylic/capric triglyceride"],
  },
  {
    id: "la-roche-toleriane",
    name: "Toleriane Double Repair Face Moisturizer",
    brand: "La Roche-Posay",
    aliases: ["lrp toleriane", "double repair", "laroche toleriane"],
    keyActives: ["Niacinamide", "Ceramide-3"],
    mainIngredients: ["Glycerin", "Dimethicone", "Prebiotic thermal water"],
  },
  {
    id: "la-roche-anthelios",
    name: "Anthelios Melt-In Milk Sunscreen SPF 60",
    brand: "La Roche-Posay",
    aliases: ["anthelios", "lrp spf", "melt in milk"],
    keyActives: ["Avobenzone", "Homosalate", "Octisalate", "Octocrylene", "Oxybenzone"],
    mainIngredients: ["Silica", "Vitamin E", "Glycerin", "Dimethicone"],
  },
  {
    id: "skinceuticals-ce-ferulic",
    name: "C E Ferulic",
    brand: "SkinCeuticals",
    aliases: ["ce ferulic", "skinceuticals vitamin c", "c e ferulic"],
    keyActives: ["L-Ascorbic acid (15%)", "Vitamin E (1%)", "Ferulic acid (0.5%)"],
    mainIngredients: ["Water", "Ethoxydiglycol", "Propylene glycol"],
  },
  {
    id: "tretinoin-generic",
    name: "Tretinoin cream (prescription)",
    brand: "Various",
    aliases: ["tretinoin", "retin-a", "stieva", "atralin"],
    keyActives: ["Tretinoin (strength varies)"],
    mainIngredients: ["Stearyl alcohol", "Isopropyl myristate", "Benzyl alcohol (often)"],
  },
  {
    id: "vanicream-gentle",
    name: "Daily Facial Moisturizer",
    brand: "Vanicream",
    aliases: ["vanicream facial", "vanicream moisturizer"],
    keyActives: ["Ceramides (barrier support; no strong actives)"],
    mainIngredients: ["Petrolatum", "Sorbitol", "Cetearyl alcohol", "Glycerin"],
  },
  {
    id: "cosrx-snail",
    name: "Advanced Snail 96 Mucin Power Essence",
    brand: "COSRX",
    aliases: ["snail mucin", "cosrx snail", "96 essence"],
    keyActives: ["Snail secretion filtrate (96%)"],
    mainIngredients: ["Panthenol", "Arginine", "Butylene glycol", "1,2-hexanediol"],
  },
  {
    id: "anua-heartleaf-toner",
    name: "Heartleaf 77% Soothing Toner",
    brand: "Anua",
    aliases: ["anua", "anua toner", "heartleaf toner", "anua heartleaf", "77 toner"],
    keyActives: ["Houttuynia cordata extract (heartleaf)"],
    mainIngredients: ["Butylene glycol", "1,2-hexanediol", "Betaine", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-heartleaf-cleansing-oil",
    name: "Heartleaf Pore Control Cleansing Oil",
    brand: "Anua",
    aliases: ["anua oil", "anua cleansing oil", "heartleaf oil", "anua pore oil"],
    keyActives: ["Heartleaf extract", "Plant oils (formula varies)"],
    mainIngredients: ["Ethylhexyl palmitate", "Sorbeth-30 tetraoleate", "Heartleaf oil blend"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-niacinamide-serum",
    name: "Niacinamide 10% + TXA 4% Serum",
    brand: "Anua",
    aliases: ["anua niacinamide", "anua txa", "anua dark spot"],
    keyActives: ["Niacinamide (10%)", "Tranexamic acid (4%)"],
    mainIngredients: ["Arbutin", "Hyaluronic acid", "Ceramide NP", "Beta-glucan"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-heartleaf-spf",
    name: "Heartleaf Silky Moisture Sun Cream SPF 50+",
    brand: "Anua",
    aliases: ["anua spf", "anua sunscreen", "heartleaf spf", "anua sun cream"],
    keyActives: ["UV filters (region-specific; check label)"],
    mainIngredients: ["Heartleaf extract", "Humectants", "Silicone/elastomer blend (often)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "good-molecules-niacinamide",
    name: "Niacinamide Brightening Toner",
    brand: "Good Molecules",
    aliases: ["good molecules niacinamide", "gm niacinamide toner"],
    keyActives: ["Niacinamide", "Arbutin (often in line)"],
    mainIngredients: ["Glycerin", "Betaine", "Licorice root extract"],
  },
  {
    id: "differin-gel",
    name: "Adapalene Gel 0.1%",
    brand: "Differin",
    aliases: ["adapalene", "differin"],
    keyActives: ["Adapalene (0.1%)"],
    mainIngredients: ["Carbomer", "Edetate disodium", "Methylparaben", "Water"],
  },
  {
    id: "neutrogena-hydro-boost",
    name: "Hydro Boost Water Gel",
    brand: "Neutrogena",
    aliases: ["hydro boost", "neutrogena gel"],
    keyActives: ["Hyaluronic acid (sodium hyaluronate)"],
    mainIngredients: ["Dimethicone", "Glycerin", "Dimethiconol", "Chlorphenesin"],
  },
  {
    id: "eltamd-uv-clear",
    name: "UV Clear Broad-Spectrum SPF 46",
    brand: "EltaMD",
    aliases: ["eltamd", "uv clear", "elta md"],
    keyActives: ["Zinc oxide", "Octinoxate (in US formula)"],
    mainIngredients: ["Niacinamide", "Hyaluronic acid", "Lactic acid (low %)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "de-protini",
    name: "Protini Polypeptide Cream",
    brand: "Drunk Elephant",
    aliases: ["drunk elephant", "protini", "de protini", "polypeptide cream"],
    keyActives: ["Signal peptides", "Pygmy waterlily extract", "Amino acids"],
    mainIngredients: ["Water", "Dicaprylyl carbonate", "Glycerin", "Sclerocarya birrea seed oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "de-cfirma",
    name: "C-Firma Fresh Day Serum",
    brand: "Drunk Elephant",
    aliases: ["c firma", "cfirma", "drunk elephant vitamin c", "c firma fresh"],
    keyActives: ["L-Ascorbic acid (15%)", "Ferulic acid", "Vitamin E"],
    mainIngredients: ["Pumpkin ferment extract", "Chronocyclin", "Hyaluronic acid"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "de-framboos",
    name: "T.L.C. Framboos Glycolic Night Serum",
    brand: "Drunk Elephant",
    aliases: ["framboos", "tlc framboos", "drunk elephant glycolic"],
    keyActives: ["Glycolic", "Tartaric", "Citric", "Salicylic acids (blend)"],
    mainIngredients: ["Raspberry extract", "Horse chestnut", "Galactoarabinan"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "de-beste",
    name: "Beste No. 9 Jelly Cleanser",
    brand: "Drunk Elephant",
    aliases: ["beste", "drunk elephant cleanser", "jelly cleanser"],
    keyActives: ["Mild surfactants (non-stripping cleanse)"],
    mainIngredients: ["Virgin marula oil", "Cantaloupe fruit extract", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "tatcha-dewy",
    name: "The Dewy Skin Cream",
    brand: "Tatcha",
    aliases: ["tatcha dewy", "dewy skin cream"],
    keyActives: ["Japanese purple rice", "Okinawa algae blend", "Hyaluronic acid"],
    mainIngredients: ["Squalane", "Glycerin", "Dimethicone", "Camellia seed oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "tatcha-rice-wash",
    name: "The Rice Wash Soft Cream Cleanser",
    brand: "Tatcha",
    aliases: ["rice wash", "tatcha cleanser", "rice cleanser"],
    keyActives: ["Japanese rice powder (physical + enzyme polish, mild)"],
    mainIngredients: ["Hyaluronic acid", "Okinawa algae", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "fenty-fat-water",
    name: "Fat Water Pore-Refining Toner Serum",
    brand: "Fenty Skin",
    aliases: ["fenty skin", "fat water", "fenty toner"],
    keyActives: ["Niacinamide", "Barbados cherry (vitamin C)"],
    mainIngredients: ["Glycerin", "Green tea", "Fig extract", "Sodium hyaluronate"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "fenty-hydra-vizor",
    name: "Hydra Vizor Invisible Moisturizer Broad Spectrum SPF 30",
    brand: "Fenty Skin",
    aliases: ["hydra vizor", "fenty spf", "fenty sunscreen"],
    keyActives: ["Avobenzone", "Homosalate", "Octisalate", "Octocrylene"],
    mainIngredients: ["Kalahari melon oil", "Niacinamide", "Hyaluronic acid"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "laneige-cream-skin",
    name: "Cream Skin Toner & Moisturizer",
    brand: "Laneige",
    aliases: ["laneige", "cream skin", "laneige toner"],
    keyActives: ["White leaf tea water", "Amino acid-rich formula"],
    mainIngredients: ["Glycerin", "Meadowfoam seed oil", "Butylene glycol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "laneige-water-mask",
    name: "Water Sleeping Mask",
    brand: "Laneige",
    aliases: ["water sleeping mask", "laneige sleeping"],
    keyActives: ["Squalane", "Pro-biotics complex", "Hyaluronic acid"],
    mainIngredients: ["Evening primrose root extract", "Apricot kernel extract", "Trehalose"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "laneige-lip-sleeping",
    name: "Lip Sleeping Mask",
    brand: "Laneige",
    aliases: ["lip sleeping mask", "laneige lip"],
    keyActives: ["Vitamin C derivative (line-dependent)", "Antioxidants"],
    mainIngredients: ["Shea butter", "Murumuru seed butter", "Wax esters"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "farmacy-green-clean",
    name: "Green Clean Makeup Removing Cleansing Balm",
    brand: "Farmacy",
    aliases: ["farmacy", "green clean", "cleansing balm"],
    keyActives: ["Sunflower + ginger root oils (cleansing)"],
    mainIngredients: ["Papaya extract", "Moringa seed oil", "Polyethylene"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "farmacy-honey-halo",
    name: "Honey Halo Ultra-Hydrating Ceramide Moisturizer",
    brand: "Farmacy",
    aliases: ["honey halo", "farmacy ceramide"],
    keyActives: ["Ceramides", "Honey blend", "Shea butter"],
    mainIngredients: ["Buckwheat honey", "Fig fruit extract", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-banana-bright",
    name: "Banana Bright 15% Vitamin C Dark Spot Serum",
    brand: "OLEHENRIKSEN",
    aliases: ["ole henriksen", "banana bright", "ole vitamin c"],
    keyActives: ["15% vitamin C complex", "PHAs (polyhydroxy acids)"],
    mainIngredients: ["Banana powder-inspired pigments", "Hyaluronic acid", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-truth-juice",
    name: "Truth Juice Daily Cleanser",
    brand: "OLEHENRIKSEN",
    aliases: ["truth juice", "ole cleanser"],
    keyActives: ["Orange fruit water", "PHAs (gentle exfoliation)"],
    mainIngredients: ["Glycerin", "Lauric acid", "Sodium cocoyl glutamate"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "sunday-good-genes",
    name: "Good Genes All-In-One Lactic Acid Treatment",
    brand: "Sunday Riley",
    aliases: ["good genes", "sunday riley lactic", "lactic acid treatment"],
    keyActives: ["Lactic acid (purified grade)"],
    mainIngredients: ["Licorice", "Lemongrass extract", "Potassium hydroxide (pH adjust)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "sunday-luna",
    name: "Luna Sleeping Night Oil",
    brand: "Sunday Riley",
    aliases: ["luna oil", "sunday riley retinol", "luna sleeping"],
    keyActives: ["Trans-retinol ester", "Blue tansy", "German chamomile"],
    mainIngredients: ["Avocado seed oil", "Chia seed oil", "Grape seed oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ptr-water-drench",
    name: "Water Drench Hyaluronic Cloud Cream Hydrating Moisturizer",
    brand: "Peter Thomas Roth",
    aliases: ["ptr", "peter thomas roth", "water drench", "cloud cream"],
    keyActives: ["Hyaluronic acid complex (30% hyaluronic acid statement, line)"],
    mainIngredients: ["Silk proteins", "Pentylene glycol", "Dimethicone"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ptr-cucumber",
    name: "Cucumber Gel Mask Extreme De-Tox Hydrator",
    brand: "Peter Thomas Roth",
    aliases: ["cucumber mask", "ptr cucumber"],
    keyActives: ["Cucumber extract", "Papaya extract (enzymatic, mild)"],
    mainIngredients: ["Aloe", "Chamomile", "Algae extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "fresh-soy",
    name: "Soy Face Cleanser",
    brand: "Fresh",
    aliases: ["fresh soy", "soy cleanser", "fresh cleanser"],
    keyActives: ["Soy proteins", "Rose water", "Borage seed oil"],
    mainIngredients: ["Cucumber extract", "Sunflower seed oil", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "fresh-rose-cream",
    name: "Rose Deep Hydration Face Cream",
    brand: "Fresh",
    aliases: ["fresh rose", "rose deep hydration"],
    keyActives: ["Damask rose extract", "Hyaluronic acid", "Evening primrose oil"],
    mainIngredients: ["Squalane", "Dimethicone", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "glow-dew-drops",
    name: "Watermelon Glow Niacinamide Dew Drops",
    brand: "Glow Recipe",
    aliases: ["glow recipe", "dew drops", "watermelon niacinamide"],
    keyActives: ["Niacinamide", "Watermelon extract", "Moringa seed oil"],
    mainIngredients: ["Hyaluronic acid", "Glycerin", "Mica (radiance)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "glow-pha-bha-toner",
    name: "Watermelon Glow PHA + BHA Pore-Tight Toner",
    brand: "Glow Recipe",
    aliases: ["watermelon toner", "pha bha toner", "glow recipe toner"],
    keyActives: ["PHA (gluconolactone)", "Willow bark (BHA-like)", "Watermelon enzymes"],
    mainIngredients: ["Cactus water", "Tea tree", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "kate-exfolikate",
    name: "ExfoliKate Intensive Exfoliating Treatment",
    brand: "Kate Somerville",
    aliases: ["exfolikate", "kate somerville", "kate exfoliating"],
    keyActives: ["Lactic acid", "Salicylic acid", "Silica (physical polish)"],
    mainIngredients: ["Pumpkin ferment", "Papaya", "Pineapple enzymes"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "kiehls-ultra-facial",
    name: "Ultra Facial Cream with Squalane",
    brand: "Kiehl's",
    aliases: ["kiehls", "kiehl's", "ultra facial cream"],
    keyActives: ["Squalane", "Glacial glycoprotein"],
    mainIngredients: ["Glycerin", "Dimethicone", "Ceramides (variant-dependent)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "kiehls-midnight",
    name: "Midnight Recovery Concentrate",
    brand: "Kiehl's",
    aliases: ["midnight recovery", "kiehl night oil"],
    keyActives: ["Evening primrose oil", "Lavender essential oil", "Squalane"],
    mainIngredients: ["Jojoba oil", "Rosa canina fruit oil", "Coriander seed oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "clinique-moisture-surge",
    name: "Moisture Surge 100H Auto-Replenishing Hydrator",
    brand: "Clinique",
    aliases: ["clinique", "moisture surge", "100h"],
    keyActives: ["Aloe bioferment", "Hyaluronic acid", "Auto-replenishing lipid-sphere tech"],
    mainIngredients: ["Dimethicone", "Glycerin", "Trehalose"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "clinique-take-day-off",
    name: "Take The Day Off Cleansing Balm",
    brand: "Clinique",
    aliases: ["take the day off", "clinique balm"],
    keyActives: ["Safflower seed oil (dissolves makeup)"],
    mainIngredients: ["Ethylhexyl palmitate", "Synthetic wax", "Polyethylene"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "philosophy-purity",
    name: "Purity Made Simple One-Step Facial Cleanser",
    brand: "Philosophy",
    aliases: ["philosophy", "purity", "purity cleanser"],
    keyActives: ["Meadowfoam seed oil", "Rosewood oil (light)"],
    mainIngredients: ["Sodium lauroamphoacetate", "Glycerin", "Sage leaf extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "caudalie-vinoperfect",
    name: "Vinoperfect Radiance Dark Spot Serum",
    brand: "Caudalie",
    aliases: ["caudalie", "vinoperfect", "caudalie vitamin c"],
    keyActives: ["Viniferine (grapevine sap extract)", "Olive squalane"],
    mainIngredients: ["Niacinamide", "Bisabolol", "Hyaluronic acid"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "drjart-ceramidin",
    name: "Ceramidin Skin Barrier Moisturizing Cream",
    brand: "Dr. Jart+",
    aliases: ["dr jart", "ceramidin", "dr jart ceramide"],
    keyActives: ["Ceramide NP", "Panthenol", "Bifida ferment lysate"],
    mainIngredients: ["Shea butter", "Glycerin", "Beeswax"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "drjart-cicapair",
    name: "Cicapair Tiger Grass Color Correcting Treatment SPF 30",
    brand: "Dr. Jart+",
    aliases: ["cicapair", "tiger grass", "dr jart cica"],
    keyActives: ["Centella asiatica (tiger grass)", "Mineral SPF"],
    mainIngredients: ["Zinc oxide", "Titanium dioxide", "Niacinamide"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "supergoop-unseen",
    name: "Unseen Sunscreen SPF 40",
    brand: "Supergoop",
    aliases: ["supergoop", "unseen sunscreen"],
    keyActives: ["Avobenzone", "Homosalate", "Octisalate", "Octocrylene"],
    mainIngredients: ["Dimethicone", "Meadowfoam seed oil", "Shea butter"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "supergoop-glowscreen",
    name: "Glowscreen SPF 40",
    brand: "Supergoop",
    aliases: ["glowscreen", "supergoop glow"],
    keyActives: ["Chemical UV filters (US formula)", "Niacinamide", "Hyaluronic acid"],
    mainIngredients: ["Mica", "Cocoa seed extract", "Sea lavender"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "fab-ultra-repair",
    name: "Ultra Repair Cream Intense Hydration",
    brand: "First Aid Beauty",
    aliases: ["first aid beauty", "fab", "ultra repair cream"],
    keyActives: ["Colloidal oatmeal", "Allantoin", "Shea butter"],
    mainIngredients: ["Eucalyptus oil (sensory; patch test)", "Glycerin", "Dimethicone"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-retinol-serum",
    name: "Retinol Youth Renewal Serum",
    brand: "Murad",
    aliases: ["murad retinol", "youth renewal"],
    keyActives: ["Retinol", "Swertia flower extract (line)"],
    mainIngredients: ["Glycerin", "Caprylic/capric triglyceride", "Hyaluronic acid"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-aha-bha-cleanser",
    name: "AHA/BHA Exfoliating Cleanser",
    brand: "Murad",
    aliases: ["murad cleanser", "murad aha bha"],
    keyActives: ["Salicylic acid", "Glycolic acid", "Jojoba beads (physical)"],
    mainIngredients: ["Sodium PCA", "Licorice extract", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "itc-confidence-cream",
    name: "Confidence in a Cream Anti-Aging Hydrating Moisturizer",
    brand: "IT Cosmetics",
    aliases: ["it cosmetics", "confidence in a cream", "itc cream"],
    keyActives: ["Collagen", "Peptides", "Niacinamide"],
    mainIngredients: ["Shea butter", "Squalane", "Ceramides"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "origins-ginzing-serum",
    name: "GinZing Into the Glow Brightening Serum",
    brand: "Origins",
    aliases: ["origins", "ginzing", "ginzing serum"],
    keyActives: ["Vitamin C (line variant)", "Niacinamide", "Ginseng"],
    mainIngredients: ["Caffeine", "Hyaluronic acid", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "origins-mega-mushroom",
    name: "Dr. Andrew Weil for Origins Mega-Mushroom Relief & Resilience Treatment Lotion",
    brand: "Origins",
    aliases: ["mega mushroom", "origins mushroom", "treatment lotion"],
    keyActives: ["Reishi mushroom", "Fermented chaga", "Lactobacillus ferment"],
    mainIngredients: ["Sodium hyaluronate", "Glycerin", "Squalane"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "shiseido-ultimune",
    name: "Ultimune Power Infusing Concentrate",
    brand: "Shiseido",
    aliases: ["shiseido", "ultimune"],
    keyActives: ["Reishi mushroom", "Iris root extract", "Ginkgo biloba"],
    mainIngredients: ["Glycerin", "Dimethicone", "Alcohol denat. (vehicle)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "el-anr",
    name: "Advanced Night Repair Synchronized Multi-Recovery Complex Serum",
    brand: "Estée Lauder",
    aliases: ["estee lauder", "advanced night repair", "anr"],
    keyActives: ["Bifida ferment lysate", "Peptides", "Hyaluronic acid"],
    mainIngredients: ["Sodium hyaluronate", "Yeast extract", "Lactobacillus ferment"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "biossance-vitamin-c-rose",
    name: "Squalane + Vitamin C Rose Oil",
    brand: "Biossance",
    aliases: ["biossance", "vitamin c rose oil", "squalane vitamin c"],
    keyActives: ["THD ascorbate (oil-soluble vitamin C)", "Squalane", "Rose oil"],
    mainIngredients: ["Chia seed oil", "Pomegranate sterols", "Tocopherol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "summer-fridays-jet-lag",
    name: "Jet Lag Mask",
    brand: "Summer Fridays",
    aliases: ["summer fridays", "jet lag mask"],
    keyActives: ["Niacinamide", "Ceramides", "Antioxidant extracts"],
    mainIngredients: ["Chestnut extract", "Vitamin E", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "tower28-sos",
    name: "SOS Daily Rescue Facial Spray",
    brand: "Tower 28",
    aliases: ["tower 28", "sos spray", "tower28"],
    keyActives: ["Hypochlorous acid (skin-soothing, line)"],
    mainIngredients: ["Water", "Sodium chloride", "Electrolytes"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ct-magic-cream",
    name: "Charlotte's Magic Cream Moisturizer",
    brand: "Charlotte Tilbury",
    aliases: ["charlotte tilbury", "magic cream", "ct magic"],
    keyActives: ["Peptide complex", "Vitamin C", "Hyaluronic acid"],
    mainIngredients: ["Shea butter", "Aloe", "Rosehip oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "rare-beauty-mist",
    name: "Always an Optimist 4-in-1 Mist",
    brand: "Rare Beauty",
    aliases: ["rare beauty", "optimist mist", "rare beauty mist"],
    keyActives: ["Niacinamide (light)", "Botanical extracts"],
    mainIngredients: ["Glycerin", "Panthenol", "Lotus extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "yttp-superfood-cleanser",
    name: "Superfood Antioxidant Cleanser",
    brand: "Youth To The People",
    aliases: ["yttp", "youth to the people", "superfood cleanser", "kale cleanser"],
    keyActives: ["Kale", "Spinach", "Green tea (cold-pressed extracts)"],
    mainIngredients: ["Glycerin", "Cocamidopropyl hydroxysultaine", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "kosas-plump-juicy",
    name: "Plump + Juicy Vegan Collagen Spray-On Serum",
    brand: "Kosas",
    aliases: ["kosas", "plump juicy", "vegan collagen spray"],
    keyActives: ["Peptides", "Probiotics", "Hyaluronic acid"],
    mainIngredients: ["Aloe leaf juice", "Glycerin", "Niacinamide"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "herbivore-lapis",
    name: "Lapis Blue Tansy Face Oil",
    brand: "Herbivore",
    aliases: ["herbivore", "lapis oil", "blue tansy"],
    keyActives: ["Blue tansy oil (azulene)", "Squalane"],
    mainIngredients: ["Jojoba seed oil", "Fractionated coconut oil", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "boscia-cleanser",
    name: "Purifying Cleansing Gel",
    brand: "boscia",
    aliases: ["boscia", "purifying cleansing gel", "boscia cleanser"],
    keyActives: ["Activated charcoal (line)", "Willow bark extract"],
    mainIngredients: ["Hydrated silica", "Glycerin", "Tea tree oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "glamglow-supermud",
    name: "Supermud Clearing Treatment",
    brand: "GLAMGLOW",
    aliases: ["glamglow", "supermud", "glam glow"],
    keyActives: ["Charcoal", "AHA/BHA acid blend", "Kaolin clay"],
    mainIngredients: ["Eucalyptus leaf", "Licorice root", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "algenist-genius",
    name: "GENIUS Ultimate Anti-Aging Cream",
    brand: "Algenist",
    aliases: ["algenist", "genius cream"],
    keyActives: ["Alguronic acid (brand algae)", "Peptides", "Vitamin C"],
    mainIngredients: ["Shea butter", "Squalane", "Apple stem cell extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "dermalogica-pre cleanse",
    name: "PreCleanse Cleansing Oil",
    brand: "Dermalogica",
    aliases: ["dermalogica", "precleanse", "pre cleanse"],
    keyActives: ["Vitamin E", "Borage seed oil", "Apricot kernel oil"],
    mainIngredients: ["Caprylic/capric triglyceride", "Lauric acid", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "dermalogica-daily-microfoliant",
    name: "Daily Microfoliant Exfoliator",
    brand: "Dermalogica",
    aliases: ["daily microfoliant", "dermalogica microfoliant"],
    keyActives: ["Rice enzymes (papain, salicylic acid micro-exfoliation)"],
    mainIngredients: ["Rice starch", "Colloidal oatmeal", "Salicylic acid"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "tula-cleanser",
    name: "The Cult Classic Purifying Face Cleanser",
    brand: "TULA Skincare",
    aliases: ["tula", "cult classic cleanser", "tula cleanser"],
    keyActives: ["Probiotic extracts", "Chicory root", "Turmeric"],
    mainIngredients: ["Glycerin", "Lactic acid (low %)", "Licorice"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "nars-aqua-glow",
    name: "Light Reflecting Moisturizer",
    brand: "NARS",
    aliases: ["nars moisturizer", "light reflecting moisturizer"],
    keyActives: ["Niacinamide", "Peptides", "Hyaluronic acid"],
    mainIngredients: ["Japanese botanical blend", "Squalane", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "milk-vegan-milk-moisturizer",
    name: "Vegan Milk Moisturizer",
    brand: "Milk Makeup",
    aliases: ["milk makeup", "vegan milk moisturizer", "milk moisturizer"],
    keyActives: ["Desert milk blend (fig, argan, oat, camelina, shea)"],
    mainIngredients: ["Squalane", "Glycerin", "Grape seed oil", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },

  /* —— Expanded Ulta / Sephora skincare shelf (representative; formulas change) —— */

  {
    id: "anua-azelaic-acid-10",
    name: "Azelaic Acid 10% Hyaluron Redness Soothing Cream",
    brand: "Anua",
    aliases: ["anua azelaic", "anua 10 azelaic", "anua redness cream"],
    keyActives: ["Azelaic acid (10%)", "Hyaluronic acid", "Panthenol"],
    mainIngredients: ["Centella asiatica", "Ceramide NP", "Beta-glucan"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-acne-cleansing-foam",
    name: "ACNEZ 365 Salicylic Acid Cleansing Foam",
    brand: "Anua",
    aliases: ["anua acne foam", "anua salicylic cleanser", "acnez 365"],
    keyActives: ["Salicylic acid (BHA)", "Heartleaf extract"],
    mainIngredients: ["Glycerin", "Tea tree leaf water", "Centella asiatica"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-peach-retinol-serum",
    name: "Peach Retinol Niacin Renewing Serum",
    brand: "Anua",
    aliases: ["anua peach", "anua peach retinol", "anua niacin renewing"],
    keyActives: ["Retinol (encapsulated, line)", "Niacinamide", "Peach extract"],
    mainIngredients: ["Adenosine", "Ceramide", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-birch-moisture-sunscreen",
    name: "Moisture Mild Sun Essence SPF 50+ PA++++",
    brand: "Anua",
    aliases: ["anua birch sun", "anua sun essence", "anua mild sun"],
    keyActives: ["Organic / inorganic UV filters (region-specific)"],
    mainIngredients: ["Betula platyphylla juice", "Hyaluronic acid", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-deep-vita-c",
    name: "Deep Vita C Capsule Cream",
    brand: "Anua",
    aliases: ["anua vita c", "deep vita c", "anua vitamin c cream"],
    keyActives: ["Vitamin C derivative (line)", "Niacinamide"],
    mainIngredients: ["Capsule beads (visual)", "Tocopherol", "Adenosine"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-quercetinol-pore-serum",
    name: "Pore Deep Clearing Serum",
    brand: "Anua",
    aliases: ["anua pore serum", "quercetinol", "anua clearing serum"],
    keyActives: ["Quercetin / flavonoid blend (line)", "Niacinamide"],
    mainIngredients: ["Heartleaf extract", "Hyaluronic acid", "Zinc PCA"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "anua-7-rice-moisturizer",
    name: "7 Rice Ceramide Hydrating Barrier Moisturizer",
    brand: "Anua",
    aliases: ["anua rice cream", "7 rice ceramide", "anua barrier cream"],
    keyActives: ["Rice bran water", "Ceramide NP", "Oryza sativa extract"],
    mainIngredients: ["Shea butter", "Squalane", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "byoma-moisturizing-gel-cream",
    name: "Moisturizing Gel Cream",
    brand: "BYOMA",
    aliases: ["byoma gel cream", "byoma moisturizer", "byoma blue"],
    keyActives: ["Tri-ceramide complex", "Niacinamide", "Green tea"],
    mainIngredients: ["Glycerin", "Squalane", "Beta-glucan"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-milky-toner",
    name: "Milky Moisture Boosting Toner",
    brand: "BYOMA",
    aliases: ["byoma toner", "byoma milky toner", "moisture boosting toner"],
    keyActives: ["Ceramides", "Squalane", "Hyaluronic acid"],
    mainIngredients: ["Oat kernel extract", "Glycerin", "Allantoin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-brightening-serum",
    name: "Brightening Serum",
    brand: "BYOMA",
    aliases: ["byoma brightening", "byoma vitamin c serum"],
    keyActives: ["Niacinamide", "Vitamin C derivative", "Licorice root"],
    mainIngredients: ["Glycerin", "Sodium hyaluronate", "Tocopherol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-creamy-jelly-cleanser",
    name: "Creamy Jelly Cleanser",
    brand: "BYOMA",
    aliases: ["byoma cleanser", "jelly cleanser byoma"],
    keyActives: ["Mild surfactants", "Green tea", "Licorice"],
    mainIngredients: ["Glycerin", "Aloe", "Betaine"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-dehydrated-skin-moisturizer",
    name: "Intense Moisture Cream for Dehydrated Skin",
    brand: "BYOMA",
    aliases: ["byoma intense moisture", "byoma dehydrated"],
    keyActives: ["Ceramides", "Squalane", "Cholesterol (barrier lipids)"],
    mainIngredients: ["Shea butter", "Glycerin", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-clarifying-serum",
    name: "Clarifying Serum",
    brand: "BYOMA",
    aliases: ["byoma clarifying", "byoma salicylic serum"],
    keyActives: ["Salicylic acid (low %)", "Niacinamide", "Zinc"],
    mainIngredients: ["Green tea", "Willow bark", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "byoma-moisturizing-rich-cream",
    name: "Moisturizing Rich Cream",
    brand: "BYOMA",
    aliases: ["byoma rich cream", "byoma barrier cream"],
    keyActives: ["Tri-ceramide complex", "Squalane"],
    mainIngredients: ["Petrolatum alternative blend", "Shea butter", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "murad-essential-c-day-moisture",
    name: "Essential-C Day Moisture Broad Spectrum SPF 30",
    brand: "Murad",
    aliases: ["murad essential c", "murad vitamin c moisturizer", "murad spf 30"],
    keyActives: ["Vitamin C (line)", "Chemical + mineral UV filters (check label)"],
    mainIngredients: ["Yeast amino acids", "Glycerin", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-city-skin-age-defense-spf50",
    name: "City Skin Age Defense Broad Spectrum SPF 50",
    brand: "Murad",
    aliases: ["murad city skin", "murad mineral spf 50", "age defense spf"],
    keyActives: ["Zinc oxide", "Titanium dioxide", "Iron oxides (tint)"],
    mainIngredients: ["Vitamin C", "Bisabolol", "Argan oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-rapid-dark-spot-correcting-serum",
    name: "Rapid Dark Spot Correcting Serum",
    brand: "Murad",
    aliases: ["murad dark spot", "rapid dark spot", "murad pigment"],
    keyActives: ["Hexylresorcinol (line)", "Glycolic acid", "Tranexamic acid (variant)"],
    mainIngredients: ["Niacinamide", "Glycerin", "Peptide"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-clarifying-cleanser",
    name: "Clarifying Cream Cleanser",
    brand: "Murad",
    aliases: ["murad clarifying cleanser", "murad cream cleanser"],
    keyActives: ["Salicylic acid", "Vitamin C (low %)"],
    mainIngredients: ["Encapsulated salicylic", "Glycerin", "Allantoin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-oil-control-mattifier",
    name: "Oil Control Mattifier SPF 15",
    brand: "Murad",
    aliases: ["murad mattifier", "murad oil control"],
    keyActives: ["Oil-absorbing microspheres", "Willow bark extract"],
    mainIngredients: ["Zinc", "Titanium dioxide", "Dimethicone"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-hydro-dynamic-quenching-essence",
    name: "Hydro-Dynamic Quenching Essence",
    brand: "Murad",
    aliases: ["murad essence", "hydro dynamic essence", "quenching essence"],
    keyActives: ["Mexican yam extract", "Hyaluronic acid"],
    mainIngredients: ["Glycerin", "Trehalose", "Urea"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-revitalixir-recovery-serum",
    name: "Revitalixir Recovery Serum",
    brand: "Murad",
    aliases: ["murad revitalixir", "recovery serum murad"],
    keyActives: ["Neuropeptide blend", "Caffeine", "Wild indigo"],
    mainIngredients: ["Squalane", "Shea butter", "Niacinamide"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-multi-vitamin-infusion-oil",
    name: "Multi-Vitamin Infusion Oil",
    brand: "Murad",
    aliases: ["murad vitamin oil", "multi vitamin oil"],
    keyActives: ["Vitamins A–F in oil-soluble forms (line claim)"],
    mainIngredients: ["Tamanu oil", "Jojoba oil", "Pomegranate seed oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "murad-outsmart-acne-clarifying",
    name: "Outsmart Acne Clarifying Treatment",
    brand: "Murad",
    aliases: ["murad outsmart", "outsmart acne"],
    keyActives: ["Salicylic acid", "Oligopeptide-10", "Amino sugars blend"],
    mainIngredients: ["Hyaluronic acid", "Glycerin", "Zinc"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "ole-strength-trainer-peptide-boost",
    name: "Strength Trainer Peptide Boost Moisturizer",
    brand: "OLEHENRIKSEN",
    aliases: ["ole strength trainer", "peptide boost ole", "strength trainer"],
    keyActives: ["Peptides", "Amino acids", "Hyaluronic acid"],
    mainIngredients: ["Squalane", "Glycerin", "Cloudberry"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-dewtopia-overnight-mask",
    name: "Dewtopia 20% Acid Night Treatment",
    brand: "OLEHENRIKSEN",
    aliases: ["dewtopia", "ole 20 acid", "dewtopia mask"],
    keyActives: ["PHA (20% blend)", "AHA (line blend)"],
    mainIngredients: ["Lactic acid", "Gluconolactone", "Aloe"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-cold-plunge-pore-mask",
    name: "Cold Plunge Pore Remedy Moisturizer",
    brand: "OLEHENRIKSEN",
    aliases: ["cold plunge", "ole cold plunge", "pore remedy ole"],
    keyActives: ["Salicylic acid", "AHAs", "Green tea"],
    mainIngredients: ["Lactic acid", "Glycerin", "Eucalyptus leaf oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-glow2oh-dark-spot-toner",
    name: "Glow2OH Dark Spot Toner",
    brand: "OLEHENRIKSEN",
    aliases: ["glow2oh", "ole dark spot toner", "glow 2 oh"],
    keyActives: ["Glycolic acid", "Lactic acid", "Salicylic acid"],
    mainIngredients: ["Licorice root", "High-potency acid blend"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-nurture-me-moisturizer",
    name: "Nurture Me Moisturizing Crème",
    brand: "OLEHENRIKSEN",
    aliases: ["nurture me", "ole nurture me", "ole pink cream"],
    keyActives: ["Vitamins A, C, E", "Omega fatty acids"],
    mainIngredients: ["Shea butter", "Sweet almond oil", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-clean-truth-foaming",
    name: "The Clean Truth Foaming Cleanser",
    brand: "OLEHENRIKSEN",
    aliases: ["clean truth cleanser", "ole foaming cleanser", "wonderbar ole"],
    keyActives: ["Orange peel oil", "PHAs"],
    mainIngredients: ["Glycerin", "Sodium cocoyl glutamate", "Licorice"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-invigorating-night-gel",
    name: "Invigorating Night Gel",
    brand: "OLEHENRIKSEN",
    aliases: ["invigorating night gel", "ole night gel", "blue gel ole"],
    keyActives: ["Alphahydroxy acids", "Eucalyptus"],
    mainIngredients: ["Glycolic acid", "Lactic acid", "Alcohol denat. (vehicle)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ole-transform-plus-phat-glow",
    name: "PHAT Glow Facial™",
    brand: "OLEHENRIKSEN",
    aliases: ["phat glow", "ole phat", "transform plus"],
    keyActives: ["PHA (polyhydroxy acid)", "Nordic birch sap"],
    mainIngredients: ["Lime pearl extract", "Chia seed oil", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "inkey-oat-cleansing-balm",
    name: "Oat Cleansing Balm",
    brand: "The INKEY List",
    aliases: ["inkey list oat", "inkey oat balm", "the inkey list cleansing"],
    keyActives: ["Colloidal oatmeal", "Sunflower seed oil"],
    mainIngredients: ["Sweet almond oil", "Cocoa seed butter", "Oat kernel oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "inkey-retinol-serum",
    name: "Retinol Serum",
    brand: "The INKEY List",
    aliases: ["inkey retinol", "the inkey list retinol"],
    keyActives: ["Retinol (1%, line)"],
    mainIngredients: ["Squalane", "Soybean oil", "Humectants"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "inkey-ha-serum",
    name: "Hyaluronic Acid Serum",
    brand: "The INKEY List",
    aliases: ["inkey ha", "inkey hyaluronic", "the inkey list ha"],
    keyActives: ["Multi-molecular hyaluronic acid"],
    mainIngredients: ["Glycerin", "Pentylene glycol", "Phenoxyethanol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "inkey-niacinamide",
    name: "Niacinamide Oil Control Serum",
    brand: "The INKEY List",
    aliases: ["inkey niacinamide", "inkey oil control"],
    keyActives: ["Niacinamide (10%)", "Zinc PCA"],
    mainIngredients: ["Dimethyl isosorbide", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "peace-out-acne-dots",
    name: "Acne Healing Dots",
    brand: "Peace Out",
    aliases: ["peace out dots", "acne dots", "pimple patches peace out"],
    keyActives: ["Salicylic acid", "Retinol (micro-dose)", "Vitamin A"],
    mainIngredients: ["Hydrocolloid", "Aloe", "Grape seed extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "peace-out-wrinkles",
    name: "Wrinkles Microneedling Anti-Wrinkle Retinol Patches",
    brand: "Peace Out",
    aliases: ["peace out wrinkles", "microneedling patches"],
    keyActives: ["Retinol", "Peptides"],
    mainIngredients: ["Sodium hyaluronate", "Adenosine", "Vitamin C"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "innisfree-green-tea-hyaluronic-serum",
    name: "Green Tea Hyaluronic Acid Hydrating Serum",
    brand: "innisfree",
    aliases: ["innisfree green tea serum", "innisfree hyaluronic"],
    keyActives: ["Green tea seed extract", "Hyaluronic acid"],
    mainIngredients: ["Panthenol", "Glycerin", "Beta-glucan"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "innisfree-dewy-glow-jelly-cream",
    name: "Dewy Glow Jelly Cream with Jeju Cherry Blossom",
    brand: "innisfree",
    aliases: ["innisfree jelly cream", "cherry blossom cream innisfree"],
    keyActives: ["Niacinamide", "Cherry blossom extract"],
    mainIngredients: ["Betaine", "Squalane", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "peach-lily-glass-skin-serum",
    name: "Glass Skin Refining Serum",
    brand: "Peach & Lily",
    aliases: ["peach and lily", "glass skin serum", "peach lily"],
    keyActives: ["Niacinamide", "Peach extract", "Meadowfoam seed oil"],
    mainIngredients: ["Hyaluronic acid", "Licorice", "Camellia"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "peach-lily-power-cocktail",
    name: "Power Cocktail Lactic Acid Repair Serum",
    brand: "Peach & Lily",
    aliases: ["power cocktail", "peach lily lactic"],
    keyActives: ["Lactic acid", "Adenosine", "Ceramides"],
    mainIngredients: ["Glycerin", "Squalane", "Cholesterol"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "ddg-alpha-beta-universal-peel",
    name: "Alpha Beta® Universal Daily Peel",
    brand: "Dr. Dennis Gross Skincare",
    aliases: ["dr dennis gross peel", "alpha beta peel", "ddg peel"],
    keyActives: ["Glycolic acid", "Salicylic acid", "Lactic acid", "Mandelic acid"],
    mainIngredients: ["Green tea", "Chamomile", "Retinol (step 2, line)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "ddg-c-collagen-serum",
    name: "C + Collagen Brighten & Firm Serum",
    brand: "Dr. Dennis Gross Skincare",
    aliases: ["ddg vitamin c", "c collagen serum", "dennis gross vitamin c"],
    keyActives: ["3-O ethyl ascorbic acid", "Collagen amino acids"],
    mainIngredients: ["Niacinamide", "CoQ10", "Squalane"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "lancome-genifique",
    name: "Advanced Génifique Youth Activating Serum",
    brand: "Lancôme",
    aliases: ["lancome genifique", "génifique", "lancome serum"],
    keyActives: ["Bifidus prebiotic fraction", "Hyaluronic acid"],
    mainIngredients: ["Glycerin", "Dimethicone", "Alcohol denat."],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "lancome-absolue-soft-cream",
    name: "Absolue Revitalizing & Brightening Soft Cream",
    brand: "Lancôme",
    aliases: ["lancome absolue", "absolue soft cream"],
    keyActives: ["Grand rose extracts", "Pro-Xylane"],
    mainIngredients: ["Shea butter", "Squalane", "Adenosine"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "versed-press-reset",
    name: "Press Restart Gentle Retinol Serum",
    brand: "Versed",
    aliases: ["versed retinol", "press restart", "versed gentle retinol"],
    keyActives: ["Encapsulated retinol", "Bakuchiol"],
    mainIngredients: ["Squalane", "Aloe", "Chamomile"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "versed-guards-up-daily-spf35",
    name: "Guards Up Daily Mineral Sunscreen Broad Spectrum SPF 35",
    brand: "Versed",
    aliases: ["versed sunscreen", "guards up", "versed spf"],
    keyActives: ["Zinc oxide", "Titanium dioxide"],
    mainIngredients: ["Niacinamide", "Sea fennel", "Moringa seed extract"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "boj-ginseng-essence",
    name: "Ginseng Essence Water",
    brand: "Beauty of Joseon",
    aliases: ["beauty of joseon ginseng", "boj ginseng", "joseon essence"],
    keyActives: ["Ginseng root water", "Niacinamide"],
    mainIngredients: ["Adenosine", "Glycerin", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "boj-revive-serum",
    name: "Revive Serum: Ginseng + Snail Mucin",
    brand: "Beauty of Joseon",
    aliases: ["boj revive", "joseon snail serum", "ginseng snail"],
    keyActives: ["Snail mucin", "Ginseng", "Niacinamide"],
    mainIngredients: ["Hyaluronic acid", "Licorice", "Adenosine"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "kosas-plump-juicy-vegan-cleanser",
    name: "Plump + Juicy Vegan Cleanser",
    brand: "Kosas",
    aliases: ["kosas cleanser", "plump juicy cleanser"],
    keyActives: ["Vegan collagen peptides", "Probiotics"],
    mainIngredients: ["Aloe", "Glycerin", "Willow bark"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "haus-labs-cleanser",
    name: "The Edge Makeup Removing Cleansing Balm",
    brand: "Haus Labs by Lady Gaga",
    aliases: ["haus labs balm", "haus labs cleansing", "lady gaga cleanser"],
    keyActives: ["Fermented arnica", "Plant oils"],
    mainIngredients: ["Sunflower seed wax", "Jojoba esters", "Squalane"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "rare-beauty-find-comfort-mist",
    name: "Find Comfort Stop & Soothe Aromatherapy Face Mist",
    brand: "Rare Beauty",
    aliases: ["rare beauty find comfort", "find comfort mist", "rare beauty soothe"],
    keyActives: ["Botanical extracts", "Bisabolol (soothing)"],
    mainIngredients: ["Glycerin", "Panthenol", "Aloe leaf juice"],
    retailers: ["ulta", "sephora"],
  },

  {
    id: "hero-mighty-patch",
    name: "Mighty Patch Original",
    brand: "Hero Cosmetics",
    aliases: ["mighty patch", "hero cosmetics", "pimple patch hero"],
    keyActives: ["Hydrocolloid (absorbs fluid)"],
    mainIngredients: ["Cellulose gum", "Polyisobutene", "Mineral oil"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "cocokind-vitamin-c-serum",
    name: "Vitamin C Glow Serum",
    brand: "cocokind",
    aliases: ["cocokind vitamin c", "cocokind glow serum"],
    keyActives: ["Sodium ascorbyl phosphate", "Sea grape caviar extract"],
    mainIngredients: ["Hyaluronic acid", "Azelaic acid derivative", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "cerave-hydrating-cleanser",
    name: "Hydrating Facial Cleanser",
    brand: "CeraVe",
    aliases: ["cerave hydrating cleanser", "cerave cream cleanser"],
    keyActives: ["Ceramides NP, AP, EOP", "Hyaluronic acid"],
    mainIngredients: ["Glycerin", "Cetearyl alcohol", "Caprylic/capric triglyceride"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "cerave-sa-cleanser",
    name: "SA Cleanser Salicylic Acid Face Wash",
    brand: "CeraVe",
    aliases: ["cerave sa cleanser", "cerave salicylic cleanser"],
    keyActives: ["Salicylic acid (0.5%)", "Niacinamide", "Ceramides"],
    mainIngredients: ["Gluconolactone (PHA)", "Hyaluronic acid", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "paula-10-azelaic",
    name: "10% Azelaic Acid Booster",
    brand: "Paula's Choice",
    aliases: ["paulas azelaic", "10 azelaic paula"],
    keyActives: ["Azelaic acid (10%)", "Licorice root"],
    mainIngredients: ["Salicylic acid (low %)", "Adenosine", "Allantoin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "supergoop-play-spf50",
    name: "PLAY Everyday Lotion SPF 50",
    brand: "Supergoop",
    aliases: ["supergoop play", "play sunscreen", "supergoop lotion spf 50"],
    keyActives: ["Homosalate", "Octisalate", "Avobenzone", "Octocrylene"],
    mainIngredients: ["Sunflower extract", "Rosemary leaf extract", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "glow-plum-plump",
    name: "Plum Plump Hyaluronic Serum",
    brand: "Glow Recipe",
    aliases: ["plum plump", "glow recipe plum", "plum hyaluronic"],
    keyActives: ["Multi-weight hyaluronic acid", "Plum extract blend"],
    mainIngredients: ["Niacinamide", "Ice willowherb", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "summer-fridays-cc-me-serum",
    name: "CC Me Vitamin C Serum",
    brand: "Summer Fridays",
    aliases: ["cc me serum", "summer fridays vitamin c"],
    keyActives: ["Two forms of vitamin C", "Peptides"],
    mainIngredients: ["Squalane", "Niacinamide", "Glycerin"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "sol-de-janeiro-bum-bum-cream",
    name: "Brazilian Bum Bum Cream",
    brand: "Sol de Janeiro",
    aliases: ["sol de janeiro", "bum bum cream", "brazilian cream"],
    keyActives: ["Guaraná extract (caffeine)", "Cupuaçu butter"],
    mainIngredients: ["Açaí oil", "Coconut oil", "Fragrance (rich)"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "makeupforever-mist",
    name: "Mist & Fix Hydrating Setting Spray",
    brand: "MAKE UP FOR EVER",
    aliases: ["mufe mist and fix", "make up for ever setting spray"],
    keyActives: ["O2 skin complex (line)", "Aloe"],
    mainIngredients: ["Glycerin", "Butylene glycol", "Panthenol"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "benefit-porefessional-hydrate",
    name: "The POREfessional: Hydrate Primer",
    brand: "Benefit Cosmetics",
    aliases: ["porefessional hydrate", "benefit hydrate primer"],
    keyActives: ["Hyaluronic acid", "Shea butter"],
    mainIngredients: ["Glycerin", "Dimethicone", "Vitamin E"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "prequel-gleanser",
    name: "Gleanser Non-Drying Glycerin Cleanser",
    brand: "Prequel",
    aliases: ["prequel cleanser", "gleanser", "glycerin cleanser"],
    keyActives: ["Glycerin", "Inulin"],
    mainIngredients: ["Glycerin", "Sodium cocoyl glycinate", "Inulin"],
    retailers: ["ulta"],
  },
  {
    id: "byoma-cream-cleanser",
    name: "Creamy Jelly Cleanser",
    brand: "BYOMA",
    aliases: ["byoma cleanser", "creamy jelly cleanser", "byoma jelly"],
    keyActives: ["Tri-ceramide complex", "Licorice root", "Green tea"],
    mainIngredients: ["Glycerin", "Ceramides", "Camellia sinensis leaf extract"],
    retailers: ["ulta"],
  },
  {
    id: "byoma-gel-cream",
    name: "Moisturizing Gel-Cream",
    brand: "BYOMA",
    aliases: ["byoma gel cream", "byoma moisturizer", "moisturizing gel cream"],
    keyActives: ["Tri-ceramide complex", "Niacinamide", "Green tea"],
    mainIngredients: ["Glycerin", "Niacinamide", "Ceramides"],
    retailers: ["ulta"],
  },
  {
    id: "bubble-slam-dunk",
    name: "Slam Dunk Hydrating Moisturizer",
    brand: "Bubble",
    aliases: ["bubble moisturizer", "slam dunk", "bubble hydrating"],
    keyActives: ["Aloe", "Hoya lacunosa flower extract", "Vitamin E"],
    mainIngredients: ["Glycerin", "Squalane", "Aloe leaf juice"],
    retailers: ["ulta"],
  },
  {
    id: "bubble-solar-mate",
    name: "Solar Mate Invisible Daily Mineral Sunscreen SPF 40",
    brand: "Bubble",
    aliases: ["bubble sunscreen", "solar mate", "bubble spf"],
    keyActives: ["Zinc oxide", "Titanium dioxide"],
    mainIngredients: ["Glycerin", "Silica", "Dimethicone"],
    retailers: ["ulta"],
  },
  {
    id: "naturium-niacinamide-serum",
    name: "Niacinamide Serum 12% Plus Zinc 2%",
    brand: "Naturium",
    aliases: ["naturium niacinamide", "naturium zinc serum", "niacinamide 12"],
    keyActives: ["Niacinamide (12%)", "Zinc PCA (2%)"],
    mainIngredients: ["Glycerin", "Panthenol", "Sodium hyaluronate"],
    retailers: ["ulta"],
  },
  {
    id: "naturium-azelaic-emulsion",
    name: "Azelaic Topical Acid 10%",
    brand: "Naturium",
    aliases: ["naturium azelaic", "azelaic topical acid", "naturium acid 10"],
    keyActives: ["Azelaic acid derivative", "Niacinamide", "Vitamin C derivative"],
    mainIngredients: ["Glycerin", "Dimethicone", "Panthenol"],
    retailers: ["ulta"],
  },
  {
    id: "naturium-multi-peptide",
    name: "Multi-Peptide Advanced Serum",
    brand: "Naturium",
    aliases: ["naturium peptide", "multi peptide advanced", "peptide serum naturium"],
    keyActives: ["Peptides", "Panthenol", "Hyaluronic acid"],
    mainIngredients: ["Glycerin", "Panthenol", "Sodium hyaluronate"],
    retailers: ["ulta"],
  },
  {
    id: "peach-lily-glass-skin",
    name: "Glass Skin Refining Serum",
    brand: "Peach & Lily",
    aliases: ["glass skin serum", "peach lily serum", "glass skin refining"],
    keyActives: ["Niacinamide", "Peptides", "Peach extract"],
    mainIngredients: ["Glycerin", "Hyaluronic acid", "Madecassoside"],
    retailers: ["ulta"],
  },
  {
    id: "peach-lily-matcha-cream",
    name: "Matcha Pudding Antioxidant Cream",
    brand: "Peach & Lily",
    aliases: ["matcha pudding", "peach lily cream", "matcha cream"],
    keyActives: ["Matcha", "Niacinamide", "Adenosine"],
    mainIngredients: ["Glycerin", "Squalane", "Camellia sinensis leaf extract"],
    retailers: ["ulta"],
  },
  {
    id: "inkey-oat-cleansing-balm",
    name: "Oat Cleansing Balm",
    brand: "The INKEY List",
    aliases: ["inkey oat balm", "oat cleansing balm", "inkey cleanser"],
    keyActives: ["Colloidal oatmeal", "Oat kernel oil"],
    mainIngredients: ["Sweet almond oil", "Oat kernel oil", "Glycerin"],
    retailers: ["sephora"],
  },
  {
    id: "inkey-omega-water-cream",
    name: "Omega Water Cream",
    brand: "The INKEY List",
    aliases: ["omega water cream", "inkey moisturizer", "inkey omega"],
    keyActives: ["Niacinamide", "Omega fatty acids", "Glycerin"],
    mainIngredients: ["Glycerin", "Niacinamide", "Betaine"],
    retailers: ["sephora"],
  },
  {
    id: "inkey-succinic-acid",
    name: "Succinic Acid Acne Treatment",
    brand: "The INKEY List",
    aliases: ["succinate acne", "inkey succinic", "acne treatment inkey"],
    keyActives: ["Succinic acid", "Salicylic acid", "Sulfur"],
    mainIngredients: ["Glycerin", "Succinic acid", "Sulfur"],
    retailers: ["sephora"],
  },
  {
    id: "biossance-squalane-oil",
    name: "100% Squalane Oil",
    brand: "Biossance",
    aliases: ["biossance squalane", "squalane oil", "100 squalane"],
    keyActives: ["Squalane"],
    mainIngredients: ["Squalane"],
    retailers: ["sephora"],
  },
  {
    id: "biossance-copper-peptide",
    name: "Squalane + Copper Peptide Rapid Plumping Serum",
    brand: "Biossance",
    aliases: ["biossance copper peptide", "copper peptide serum", "rapid plumping"],
    keyActives: ["Copper peptides", "Hyaluronic acid", "Squalane"],
    mainIngredients: ["Glycerin", "Squalane", "Sodium hyaluronate"],
    retailers: ["sephora"],
  },
  {
    id: "first-aid-beauty-oat-toner",
    name: "Ultra Repair Wild Oat Hydrating Toner",
    brand: "First Aid Beauty",
    aliases: ["fab oat toner", "wild oat toner", "first aid beauty toner"],
    keyActives: ["Colloidal oatmeal", "Honey", "Hyaluronic acid"],
    mainIngredients: ["Glycerin", "Avena sativa kernel extract", "Sodium hyaluronate"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "aveeno-oat-gel",
    name: "Calm + Restore Oat Gel Moisturizer",
    brand: "Aveeno",
    aliases: ["aveeno oat gel", "calm restore moisturizer", "aveeno calm restore"],
    keyActives: ["Prebiotic oat", "Feverfew"],
    mainIngredients: ["Glycerin", "Dimethicone", "Avena sativa kernel flour"],
    retailers: ["ulta"],
  },
  {
    id: "eucerin-urea-repair",
    name: "UreaRepair Plus 5% Urea Lotion",
    brand: "Eucerin",
    aliases: ["eucerin urea", "urea repair", "5% urea lotion"],
    keyActives: ["Urea (5%)", "Ceramides", "Natural moisturizing factors"],
    mainIngredients: ["Urea", "Glycerin", "Ceramides"],
  },
  {
    id: "lrp-cicaplast-b5",
    name: "Cicaplast Balm B5",
    brand: "La Roche-Posay",
    aliases: ["cicaplast", "cicaplast b5", "lrp balm"],
    keyActives: ["Panthenol", "Madecassoside", "Shea butter"],
    mainIngredients: ["Dimethicone", "Shea butter", "Panthenol"],
    retailers: ["ulta"],
  },
  {
    id: "tower28-sos-serum",
    name: "SOS Intensive Redness Relief Serum",
    brand: "Tower 28",
    aliases: ["tower 28 serum", "sos serum", "redness relief serum"],
    keyActives: ["Hypochlorous acid support", "Azelaic acid derivative", "Allantoin"],
    mainIngredients: ["Glycerin", "Allantoin", "Bisabolol"],
    retailers: ["sephora"],
  },
  {
    id: "elf-holy-hydration-cleanser",
    name: "Holy Hydration! Daily Cleanser",
    brand: "e.l.f. SKIN",
    aliases: ["elf cleanser", "holy hydration cleanser", "elf daily cleanser"],
    keyActives: ["Hyaluronic acid", "Ceramides"],
    mainIngredients: ["Glycerin", "Cocamidopropyl betaine", "Ceramides"],
    retailers: ["ulta"],
  },
  {
    id: "elf-holy-hydration-cream",
    name: "Holy Hydration! Face Cream",
    brand: "e.l.f. SKIN",
    aliases: ["elf face cream", "holy hydration cream", "elf moisturizer"],
    keyActives: ["Hyaluronic acid", "Niacinamide", "Peptides"],
    mainIngredients: ["Glycerin", "Dimethicone", "Squalane"],
    retailers: ["ulta"],
  },
  {
    id: "hero-lightning-wand",
    name: "Lightning Wand Dark Spot Brightening Serum",
    brand: "Hero Cosmetics",
    aliases: ["hero lightning wand", "hero dark spot", "lightning wand"],
    keyActives: ["Tranexamic acid", "Vitamin C derivative", "Licorice root"],
    mainIngredients: ["Glycerin", "Tranexamic acid", "Licorice root extract"],
    retailers: ["ulta"],
  },
  {
    id: "sunday-b3-nice",
    name: "B3 Nice 10% Niacinamide Serum",
    brand: "Sunday Riley",
    aliases: ["b3 nice", "sunday riley niacinamide", "b3 nice serum"],
    keyActives: ["Niacinamide (10%)", "Trans-resveratrol", "EGCG"],
    mainIngredients: ["Glycerin", "Niacinamide", "Green tea extract"],
    retailers: ["ulta", "sephora"],
  },
  {
    id: "kosas-good-body-skin",
    name: "Good Body Skin AHA + Enzyme Exfoliating Body Wash",
    brand: "Kosas",
    aliases: ["kosas body wash", "good body skin", "aha enzyme body wash"],
    keyActives: ["AHAs", "Fruit enzymes", "Allantoin"],
    mainIngredients: ["Glycerin", "Lactic acid", "Allantoin"],
    retailers: ["sephora"],
  },
];

function normalize(s: string): string {
  return s.toLowerCase().trim().replace(/\s+/g, " ");
}

function haystack(p: CatalogProduct): string {
  const retail = (p.retailers ?? []).join(" ");
  return normalize(
    [
      p.brand,
      p.name,
      ...p.aliases,
      ...p.keyActives,
      ...p.mainIngredients,
      retail,
    ].join(" ")
  );
}

export function searchCatalog(query: string, limit = 8): CatalogProduct[] {
  const q = normalize(query);
  if (q.length < 1) return [];

  const scored: { product: CatalogProduct; score: number }[] = [];

  for (const p of PRODUCT_CATALOG) {
    const h = haystack(p);
    if (!h.includes(q)) continue;

    let score = 0;
    const display = normalize(`${p.brand} ${p.name}`);
    const nameL = p.name.toLowerCase();
    const brandL = p.brand.toLowerCase();

    if (display.startsWith(q)) score += 12;
    else if (h.startsWith(q)) score += 9;

    if (nameL.startsWith(q)) score += 6;
    else if (nameL.includes(q)) score += 3;

    if (brandL.startsWith(q)) score += 5;
    else if (brandL.includes(q)) score += 2;

    for (const a of p.aliases) {
      const al = a.toLowerCase();
      if (al.startsWith(q)) score += 4;
      else if (al.includes(q)) score += 1;
    }

    scored.push({ product: p, score });
  }

  scored.sort((a, b) => {
    if (b.score !== a.score) return b.score - a.score;
    return `${a.product.brand} ${a.product.name}`.localeCompare(
      `${b.product.brand} ${b.product.name}`
    );
  });

  return scored.slice(0, limit).map((s) => s.product);
}

export function getCatalogProductById(
  id: string
): CatalogProduct | undefined {
  return PRODUCT_CATALOG.find((p) => p.id === id);
}

/** Distinct brand names in the reference catalog (sorted), for quiz / filters. */
export function getCatalogBrands(): string[] {
  const skip = new Set(["various"]);
  const brands = new Set<string>();
  for (const p of PRODUCT_CATALOG) {
    const b = p.brand.trim();
    if (b && !skip.has(b.toLowerCase())) brands.add(b);
  }
  return [...brands].sort((a, b) => a.localeCompare(b));
}

export function formatProductNotes(p: CatalogProduct): string {
  const actives = p.keyActives.join(", ");
  const mains = p.mainIngredients.join(", ");
  let text = `Key actives: ${actives}\nMain ingredients (representative): ${mains}`;
  if (p.retailers?.length) {
    const labels = p.retailers.map((r) => (r === "ulta" ? "Ulta" : "Sephora"));
    text += `\nOften sold at: ${labels.join(" · ")}`;
  }
  return text;
}
